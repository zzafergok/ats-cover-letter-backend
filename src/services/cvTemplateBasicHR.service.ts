  ...
  async generatePDF(data: CVBasicHRData): Promise<Buffer> {
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (data.objective) {
          this.addSectionHeader(doc, 'OBJECTIVE', yPosition, greenColor);
          yPosition += 25;
        ...
        if (data.experience && data.experience.length > 0) {
          this.addSectionHeader(doc, 'EXPERIENCE', yPosition, greenColor);
          yPosition += 25;
        ...
        if (data.education && data.education.length > 0) {
          this.addSectionHeader(doc, 'EDUCATION', yPosition, greenColor);
          yPosition += 25;
        ...
        // Communication Section - exactly like template
        if (data.communication) {
          ...
          this.addSectionHeader(doc, 'COMMUNICATION', yPosition, greenColor);
          yPosition += 25;
        ...
        // Leadership Section - exactly like template
        if (data.leadership) {
          // Check if section fits on current page
          const sectionMinHeight = 60; // Header + minimum content
          if (yPosition + sectionMinHeight > 720) {
          ...
          this.addSectionHeader(doc, 'LEADERSHIP', yPosition, greenColor);
          yPosition += 25;
        ...
        if (data.references && data.references.length > 0) {
          ...
          this.addSectionHeader(doc, 'REFERENCES', yPosition, greenColor);
          yPosition += 25;
